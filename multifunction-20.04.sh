...
# define variables:
export WKDIR="$(readlink -f "$(dirname "$0")")" # Working directory for script
...
export VERSION='12' # version number of the respun distro
export UBURELEASE='' # Release Year.Month for Ubuntu distros
export UBUCODE='bookworm' # Ubuntu or Debian codename
export MINTRELEASE=''
export MINTCODE='' # Mint codename
export DISTRIBID='Debian' # LinuxMint or Ubuntu or Debian
...
export DISTROURL='https://respunlinux.org' # url for the respin's web page
export FLAVOUR='Debian' # /etc/casper.conf flavour
export HOST='debian' # /etc/casper.conf host in the respun distro
...
extract_ubuntu() {
    ...
    dd if="$ISONAME" bs=1 count=512 of=utils/"$MBR_FILE"
    mkdir mnt
    [[ -d "utils" ]] || mkdir utils
